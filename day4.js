// const roomStrings = `aaaaa-bbb-z-y-x-123[abxyz]
// a-b-c-d-e-f-g-h-987[abcde]
// not-a-real-room-404[oarel]
// totally-real-room-200[decoy]`;

const roomStrings = `gbc-frperg-pubpbyngr-znantrzrag-377[rgbnp]
...
function countsForString(str) {
  ...
  for (var p in counts) {
    countArray.push({ch: p, count: counts[p]});
  }
...
function isRoomValid(room) {
  const firstFive = room.counts.slice(0, 5).map(function(info) {
    return info.ch;
  }).join('');
  // console.log(firstFive, room.checksum);
...
function rotCharN(n) {
  return function(ch) {
    ...
    const charcode = (((ch.charCodeAt(0) - 97) + n) % 26) + 97;
    return String.fromCharCode(charcode);
  }
}

function rotStrN(n) {
  const rot = rotCharN(n);

  return function(str) {
    return str.split('').map(rot).join('');
  }
}

function decryptRoom(room) {
  const rot = rotStrN(room.id);
  return rot(room.name);
}

const rooms = roomStrings.split('\n').map(function(str) {
  const bracketIndex = str.lastIndexOf('[');
  const lastDashIndex = str.lastIndexOf('-', bracketIndex);

  const checksum = str.substr(bracketIndex+1, 5);
  const id = parseInt(str.substring(lastDashIndex+1, bracketIndex));
  const name = str.substring(0, lastDashIndex);

  return {
    id: id,
    checksum: checksum,
    name: name,
    counts: countsForString(name.replace(/-/g, ''))
  }
});

const sum = rooms.reduce(function(sum, room) {
  if (isRoomValid(room)) {
    // console.log(room.name, 'is valid');
    console.log(decryptRoom(room), room.id);
    return sum + room.id;
  }
