...
function getMarkerInfo(str, start) {
  ...
  const charcount = parseInt(str.substring(start, i))
  ...
  const repeat = parseInt(str.substring(startSecond, i));
...
function decompressGetLength(str) {
  let decompressed = [];

  let lastEnd = 0;
  let markerStart = str.indexOf('(', lastEnd);
  ...
  while (markerStart !== -1) {
    if (markerStart > lastEnd) {
      decompressed.push(markerStart - lastEnd);
    }

    const markerInfo = getMarkerInfo(str, markerStart + 1);

    if (markerInfo) {
      lastEnd = markerInfo.end + 1; // .end points to the ')'
      const repeated = str.substr(lastEnd, markerInfo.charcount);
      ...
      // part 1:
      // decompressed.push(markerInfo.repeat * repeated.length);

      // part 2:
      decompressed.push(decompressGetLength(repeated) * markerInfo.repeat);

      lastEnd += repeated.length;
    } else {
      lastEnd = markerStart + 1;
    }

    markerStart = str.indexOf('(', lastEnd);
  }

  if (lastEnd < str.length) {
    decompressed.push(str.length - lastEnd);
  }

  const len = decompressed.reduce(sum, 0);
...
const total = strings.split('\n').map(decompressGetLength).reduce(sum, 0);
