...
const demo = `abba[mnop]qrst
abcd[bddb]xyyx
aaaa[qwer]tyui
ioxxoj[asdfgh]zxcvbn`.split('\n');

const real = `wysextplwqpvipxdv[srzvtwbfzqtspxnethm]syqbzgtboxxzpwr[kljvjjkjyojzrstfgrw]obdhcczonzvbfby[svotajtpttohxsh]cooktbyumlpxostt
...
function splitParts(ip) {
  const normParts = [];
  const hyperParts = [];

  let start = 0;
  let openBracketIndex = ip.indexOf('[');

  while (openBracketIndex !== -1) {
    let closeBracketIndex = ip.indexOf(']', openBracketIndex);

    normParts.push(ip.substring(start, openBracketIndex));
    hyperParts.push(ip.substring(openBracketIndex + 1, closeBracketIndex));
    start = closeBracketIndex + 1;

    openBracketIndex = ip.indexOf('[', start);
  }

  if (start < ip.length) {
    normParts.push(ip.substring(start));
  }
...
function checkPartAbba(part) {
  const len = part.length - 4 + 1;
...
function checkPartNotAbba(part) {
  return !checkPartAbba(part);
}

function checkIpPartsAbba(ipParts) {
  return ipParts.norms.some(checkPartAbba) && ipParts.hypers.every(checkPartNotAbba);
}

// part 2 specifics

function addPartXyx(xyxs, part) {
  const len = part.length - 3 + 1;

  for (let i = 0; i < len; i++) {
    if (part[i] === part[i+2] && part[i] !== part[i+1]) {
      xyxs.push(part.substr(i, 3));
    }
  }
...
function invertAba(aba) {
  return [aba[1], aba[0], aba[1]].join('');
}

function getAbasAndBabs(ipParts) {
  const abas = ipParts.norms.reduce(addPartXyx, []);
  const babs = ipParts.hypers.reduce(addPartXyx, []);

  for (let i = 0; i < abas.length; i++) {
    if (babs.indexOf(invertAba(abas[i])) !== -1) {
      return true;
    }
  }
...
// rejoin back for both

const ipsParts = ips.map(splitParts);

const sumTls = ipsParts
  .filter(checkIpPartsAbba)
  .length;

const sumSsl = ipsParts
  .filter(getAbasAndBabs)
  .length;

console.log('tls capable:', sumTls);
console.log('ssl capable:', sumSsl);
