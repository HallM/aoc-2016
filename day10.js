...
Bot.prototype.takeNumber = function(num) {
  if (this.value1 === null) {
    this.value1 = num;
    ...
    if (this.isSpecialBot()) {
      console.log('Part 1 bot number:', this.botNumber);
    }
  }
};

Bot.prototype.isSpecialBot = function() {
  return (this.value1 === 61 && this.value2 === 17) || (this.value1 === 17 && this.value2 === 61);
};
...
Bot.prototype.update = function() {
  ...
  const highRecvBot = bots[cmd.h];
  const lowRecvBot = bots[cmd.l];

  const low = Math.min(this.value1, this.value2);
  const high = Math.max(this.value1, this.value2);

  this.isReady = false;
  highRecvBot.takeNumber(high);
  lowRecvBot.takeNumber(low);
  return false;
};

function Output(botNumber) {
  this.botNumber = botNumber;
  this.value1 = null;
}

Output.prototype.takeNumber = function(num) {
  this.value1 = num;
  return false;
};
...
Input.prototype.procAll = function() {
  this.cmdQueue.forEach(function(cmd) {
    const recvBot = bots[cmd.bot];
    const num = cmd.num;

    recvBot.takeNumber(num);
  });
};

const cmdstring = `bot 147 gives low to bot 67 and high to bot 71
...
cmdstring.split('\n').forEach(function(str) {
  ...
  if (parts[0] === 'bot') {
    const giver = parts[1];
    const isLowOut = parts[5] === 'output';
    const isHighOut = parts[10] === 'output';

    const lowrecv = (isLowOut ? 'o' : '') + parts[6];
    const highrecv = (isHighOut ? 'o' : '') + parts[11];

    if (!bots[lowrecv]) {
      bots[lowrecv] = isLowOut ? new Output(lowrecv) : new Bot(lowrecv);
      allbots.push(bots[lowrecv]);
    }
    if (!bots[highrecv]) {
      bots[highrecv] = isHighOut ? new Output(highrecv) : new Bot(highrecv);
      allbots.push(bots[highrecv]);
    }
    if (!bots[giver]) {
      bots[giver] = new Bot(giver);
      allbots.push(bots[giver]);
    }
    ...
    bots[giver].cmdQueue.push(cmd);
  } else {
    const value = parseInt(parts[1]);
    const recv = parts[5];

    if (!bots[recv]) {
      bots[recv] = new Bot(recv);
      allbots.push(bots[recv]);
    }
    const cmd = {
      num: value,
      bot: recv
    };
    input.cmdQueue.push(cmd);
  }
});

input.procAll();

while (allbots.length) {
  allbots = allbots.filter(function(bot) {
    return bot.update();
  });
}

const part2 = bots['o0'].value1 * bots['o1'].value1 * bots['o2'].value1;
console.log('Part 2 value:', part2);
